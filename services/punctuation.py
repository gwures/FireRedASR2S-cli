...
class PunctuationService:
    def __init__(self, config: Dict):
        ...
        self.prompt_template = config.get(
            "prompt",
            "请为以下无标点文本添加适当的标点符号，只返回加标点后的文本，不要其他解释：\n{text}",
        )
    ...
    def _get_async_client(self):
        if self._async_client is None:
            ...
            self._async_client = openai.AsyncOpenAI(
                api_key=self.api_key, base_url=base_url, timeout=self.max_timeout
            )
    ...
    async def add_punctuation_async(self, text: str) -> str:
        ...
        try:
            ...
            if response.choices and len(response.choices) > 0:
                ...
                logger.debug(
                    f"标点添加成功: {text[:20]}... -> {punctuated_text[:20]}..."
                )
            ...
            logger.warning(f"openai库未安装，无法添加标点: {e}")
            return None
...
def create_punctuation_service(config: Dict) -> Optional[PunctuationService]:
    ...
    try:
        ...
        logger.info(
            f"标点服务已创建，使用endpoint: {service.endpoint}, model: {service.model}"
        )
        return service
    except Exception as e:
