...
class ChineseCharEnglishSpmTokenizer:
    ...
    def tokenize(self, text, replace_punc=True):
        ...
        tokens = []
        if replace_punc:
            text = self._PUNC_PATTERN.sub(" ", text)
        parts = self._CHINESE_CHAR_PATTERN.split(text.strip())
        parts = [p for p in parts if len(p.strip()) > 0]
        for part in parts:
            if self._CHINESE_CHAR_PATTERN.fullmatch(part) is not None:
                tokens.append(part)
            else:
                if self.sp:
        ...
        while i < len(timestamp):
            ...
            if token.startswith(self.SPM_SPACE):
                ...
                while next_i < len(timestamp):
                    ...
                    if self._ENGLISH_PATTERN.match(next_token):
                        token += next_token
