...
class FireRedAsr2System:
    def __init__(self, config):
        c = config
        self.vad = (
            FireRedVad.from_pretrained(c.vad_model_dir, c.vad_config)
            if c.enable_vad
            else None
        )
        self.asr = FireRedAsr2.from_pretrained(
            c.asr_type, c.asr_model_dir, c.asr_config
        )
        self.config = config
    ...
    def process(self, wav_path, uttid="tmpid"):
        ...
        for j, (start_s, end_s) in enumerate(vad_segments):
            ...
            if (
                len(batch_asr_uttid) < self.config.asr_batch_size
                and j != len(vad_segments) - 1
            ):
                continue
            ...
            batch_asr_results = [
                a
                for a in batch_asr_results
                if not _BLANK_SIL_SEARCH_PATTERN.search(a["text"])
            ]
        ...
        vad_segments_ms = [
            (int(s * 1000), int(e * 1000)) for s, e in vad_result["timestamps"]
        ]
