...
from .constants import (FRAME_LENGTH_MS, FRAME_LENGTH_S, FRAME_SHIFT_MS,
                        FRAME_SHIFT_S)
...
class VadPostprocessor:
    ...
    def _process_optimized(self, raw_probs: List[float]) -> List[int]:
        n = len(raw_probs)
        if n == 0:
            return []
            ...
            for t, decision in enumerate(decisions):
                if t > 0 and decisions[t-1] == 1 and decision == 0 and silence_start is None:
                    ...
                    if silence_frame < self.merge_silence_frame:
                        decisions[silence_start:t] = [1] * silence_frame
                    silence_start = None
        
        if self.extend_speech_frame > 0:
            decisions = self._extend_speech_segments_fast(decisions, n)
        
        if self.max_speech_frame > 0:
            speech_segments = self._extract_speech_segments(decisions)
            for start_frame, end_frame in speech_segments:
                dur_frames = end_frame - start_frame
                if dur_frames > self.max_speech_frame:
                    segment_probs = raw_probs[start_frame:end_frame]
    ...
    def _extend_speech_segments_fast(self, decisions: List[int], n: int) -> List[int]:
        decisions_np = np.array(decisions, dtype=np.int8)
