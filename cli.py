...
from services.punctuation import create_punctuation_service
...
SUPPORTED_AUDIO_EXTENSIONS = {
    ...
    '.opus', '.amr', '.ape', '.aiff', '.aif', '.au', '.bwf',
    '.ac3', '.dts', '.eac3', '.thd', '.truehd',
    '.mp2', '.vqf', '.sd2',
    '.mp4', '.avi', '.mkv', '.mov', '.wmv', '.webm',
    '.flv', '.ts', '.mts', '.m2ts', '.vob', '.3gp', '.mxf', '.asf',
    '.rmvb', '.rm', '.ogv', '.f4v', '.m4v', '.divx', '.xvid'
}
...
async def process_files_async(files: List[Path], args):
    ...
    worker_config = {
        ...
        "max_vad_batch_size": max_vad_batch_size
    }
    ...
    try:
        ...
        final_results, perf_stats = asr_system.batch_process(audio_files, max_batch_dur_s=max_batch_dur_s, max_workers=8)
        ...
        for i, result in enumerate(final_results):
            ...
            completed_count += 1
            output_info = str(file_output_dir) if not unified_output_dir else str(file_output_dir)
            print_progress(f"[{completed_count}/{len(final_results)}] 完成: {original_file.name} (音频: {audio_dur:.2f}s) -> {output_info}")
        ...
        vad_s = perf_stats['timing']['vad_s']
        asr_s = perf_stats['timing']['asr_s']
        merge_s = perf_stats['timing']['merge_s']
        ...
        print_progress(f"各阶段用时: VAD={format_time(vad_s)}, "
                     f"ASR={format_time(asr_s)}, "
                     f"Merge={format_time(merge_s)}")
        print_progress(f"总处理用时: {perf_stats['total_processing_s']:.2f}s")
        print_progress(f"RTF (不含ffmpeg): {perf_stats['rtf']:.3f}x")
        print_progress("=" * 60)
        ...
        if punctuation_service is not None and output_files_info:
            print_progress(f"开始并发对 {len(output_files_info)} 个文本文件进行标点处理...")
            
            texts = [file_info["text"] for file_info in output_files_info]
            punctuated_texts = await punctuation_service.add_punctuation_batch_async(texts)
            ...
            for file_info, punctuated_text in zip(output_files_info, punctuated_texts):
                try:
                    ...
                    punct_count += 1
                    print_progress(f"[{punct_count}/{len(output_files_info)}] 标点完成: {original_file.name} -> {output_txt_pc.name}")
                except Exception as e:
                    logger.warning(f"标点结果写入失败 {file_info['original_file'].name}: {e}")
            
            print_progress("=" * 60)
            print_progress(f"标点处理完成: {punct_count}/{len(output_files_info)}")
